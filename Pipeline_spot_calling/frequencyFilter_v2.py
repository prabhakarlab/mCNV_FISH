...
def butter2d(low_cut: Union[float, None] = None,
    ...
    return frequency_mask.astype(np.float32)

def butter3d(low_cut: Union[float, None] = None,
             high_cut: Union[float, None] = None,
             z_high_cut_scaler: int = 5,
             order: int = 1,
             filter_path: str = None,
             use_existing: bool = True,
             save: bool = True,
    ...
    high_cut:
        higher frequency cut - acts as low pass filter
    z_high_cut:
        higher frequency cut in the z-direction. Has a default value specified below.       
    filter_path: str
        directory where filters are saved / will be saved
    use_existing: bool
        whether to use existing filter (if found)
    ...
    if high_cut is not None:
        filename += f"_highcut_{high_cut}"
    filename += f"_{zdim}_{ydim}_{xdim}.npy"
    ...
    # find mid-point (must -0.5 to match pixel center position)
    z_mid, y_mid, x_mid = zdim / 2, ydim / 2 - 0.5, xdim / 2 - 0.5
    if verbose:
        print(f"Mid point of filter:z = {z_mid:.3f},  y = {y_mid:.3f}, x = {x_mid:.3f}")
    
    zdim_scale = z_high_cut_scaler*zdim
    grid = np.mgrid[0:zdim_scale:int(z_high_cut_scaler), 0:ydim, 0:xdim]  # grid[0] is z-coordinate, grid[1] is y-coordinate, , grid[2] is x-coordinate

    # distance_to_mid = np.sqrt((grid[0] - z_mid) ** 2 + (grid[1] - y_mid) ** 2 + (grid[2] - x_mid) ** 2)
    
    distance_to_mid = np.sqrt((grid[0] - grid[0]) ** 2 + (grid[1] - y_mid) ** 2 + (grid[2] - x_mid) ** 2)
